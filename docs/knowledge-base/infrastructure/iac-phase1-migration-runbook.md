...
| `GCP_CREDENTIALS` | Secret | For Terraform GCP provider |
...
```powershell
# 1. GCP SAs need keys generated after creation
gcloud iam service-accounts keys create keys/gcs-key.json \
  --iam-account=gcs-storage-service@pinyin-pal-831.iam.gserviceaccount.com
# (Repeat for tts-service, gemini-service)
# Convert to one-line format for env vars
