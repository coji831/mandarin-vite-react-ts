---
purpose: NestJS 11 shell-swap — full-scoped serial Epic 24 runs FIRST to completion (15 stories), then epics 25–28 land on NestJS; 24-16 BLOCKING pre-merge release gate
status: completed
...
**Status:** ✅ Completed — full-scoped serial Epic 24 shipped all **15 stories** to completion (24-1 P0-1 stopgap → 24-15 deployment cutover); NestJS 11 is the **production entry** (`start`/`railway.toml`/`Procfile` → `node dist/nest/main.js`), the Express surface is deleted (no dual-mode), `openapi.yaml` reconciled to the 7-path System/Auth/TTS surface, `/api-docs` + `/api-docs.json` served from Nest, and the post-flip verification recorded in `verification-artifacts/release-safety-gate-24-14.md`; all absorbed scope landed (P0-1, guest identity, `SrsCardState` additive), quiz-FE fixes C-declared (26) — serial model complete, epics 25–28 land on NestJS after (D10; re-ratification `ad9c6fba`, D9→D10 resolution `8d840398`); **follow-on stories (post-close, re-sliced 2026-08-23):** **24-16 (Docs-Truth Close + API Docs Expansion)** — **BLOCKING pre-merge FIRST** (all docs scope), BR/IMP authored 2026-08-23, status **in-progress** · **24-17 (Deployment Setup (PR Env Scope) + Env-Isolation Hardening + IaC)** — **pre-merge SECOND** (deploy execution with the **two-fold merge gate** — smoke green + R1/R2 upserts verified; the rollback model was retired → single-page note in `deployment.md` §Rollback — + R1/R2/R3/R6 env-isolation hardening + IaC + owner validations; post-merge bugs → hotfix on main), BR/IMP authored 2026-08-23, re-scoped 2026-08-24, **superseded 2026-08-25 by the approved final plan** (Architect, owner-approved), status **in-progress**

**Last Update:** August 25, 2026

> **Story docs:** all 15 stories are fully authored (BR + IMP), each closed at the point it ran; commit hashes filled at epic close. **24-16** (Docs-Truth Close + API Docs Expansion) is the **BLOCKING pre-merge FIRST** story (docs-only) and **24-17** (Deployment Setup (PR Env Scope) + Env-Isolation Hardening + IaC) the **pre-merge SECOND** (deploy execution with the **two-fold merge gate** — smoke green + R1/R2 upserts verified; the rollback model was retired → single-page note in `deployment.md` §Rollback; + R1/R2/R3/R6 hardening + IaC + owner validations; post-merge bugs → hotfix on main) — both BR/IMP authored 2026-08-23 (24-17 re-scoped 2026-08-24, superseded 2026-08-25 by the approved final plan), status **in-progress**, both before the single merge. See the epic [IMP README](../../issue-implementation/epic-24-nestjs-shell-migration/README.md) for per-story status + impl hashes; each story's BR is linked in the User Stories list below.
...
This epic consists of the following user stories (full 15-story serial breakdown per the Architect's serial re-scope; slugs match the story file names — **all 15 stories link to authored BRs** and are **completed**, each closed at the point it ran; impl commit hashes are filled at epic close in the epic [IMP README](../../issue-implementation/epic-24-nestjs-shell-migration/README.md); **follow-on stories after the 15-story serial scope (re-sliced 2026-08-23; 24-17 further re-scoped 2026-08-24, superseded 2026-08-25 by the approved final plan, which also retired the rollback model → two-fold merge gate):** 24-16 Docs-Truth Close + API Docs Expansion (BLOCKING pre-merge FIRST, docs-only) + 24-17 Deployment Setup (PR Env Scope) + Env-Isolation Hardening + IaC (pre-merge SECOND — deploy execution with the two-fold merge gate + R1/R2/R3/R6 hardening + IaC + owner validations; post-merge bugs → hotfix on main) — both in-progress, see items 16–17 below):
...
16. **24-16 — Docs-Truth Close + API Docs Expansion** ([BR](story-24-16-release-prep.md)) **_(BLOCKING pre-merge FIRST — all docs scope; the docs side of the merge-PR release prep)_**

- As a documentation specialist, I want the docs-truth closed (system map + coverage + index READMEs + Nest-rewritten workflow/backend-dev instructions + `validateConfig`-aligned env docs + the 24-15 hash `5834a51e` filled + `design.md` modernized to 15 modules) and the **openapi expanded to the full ~54-route surface** (63 registered incl. shadowed, reconciled against `ROUTE_PATTERNS` + the parity harness) with the `apps/backend/docs/api/*` per-domain specs reconciled/completed, so that the docs truthfully describe the shipped NestJS backend and the API reference covers every live route.

17. **24-17 — Deployment Setup (PR Env Scope) + Env-Isolation Hardening + IaC** ([BR](story-24-17-deployment-setup-pr-env.md)) **_(pre-merge SECOND — the approved final plan (Architect, owner-approved 2026-08-25): five workstreams A–E; post-merge bugs handled as hotfixes on main)_**

- As a release captain, I want to execute the production release across five workstreams — **A deploy execution** (merge the PR with the **two-fold merge gate** — `preview.yml` smoke green + **R1/R2 upserts verified** → Railway prod deploy → post-flip prod smoke + T+0→24h watch → the **T+0 rollback-decision point** per `deployment.md` §Rollback), **B deployment enablers** (the **single-page rollback note** in `deployment.md` §Rollback — redeploy-previous + `git revert`; the former two-layer pinned-tag model + runbook are retired — plus the **post-deploy smoke job in `preview.yml`** (health + booleans, auth register/login/refresh/me, guest phase-gate shape, 4xx envelope), and the config normalizations (root `engines` `>=24`, `terraform/main.tf`/`eslint.config.js` comments)), **C env-isolation hardening** (R1 per-env JWT + `JwtService` env claim + prod rotation T+0; R2 `HEALTH_PROBE_EXTERNAL=false` probe gate + per-env GCP → preview SA `preview-service` + sandbox bucket `pinyin-pal-preview-data`; R3 Neon `parent_branch: demo` non-blocking; R6 backup gate doc-only; R4/R5 deferred Phase-2), **D IaC** (`terraform/README.md` + additive preview SA + sandbox bucket + `env-isolation.md` extraction + runbook/deployment NestJS refresh), **E owner external validations** (incl. the new upsert/secret/demo checks) — so that the NestJS production release ships, verifies, and rolls back safely with per-environment isolation — post-merge bugs are handled as hotfixes on main (separate concern, not in 24-17).
...
- See the epic [IMP README](../../issue-implementation/epic-24-nestjs-shell-migration/README.md) for per-story technical ACs + impl commit hashes — all 15 serial stories are fully authored and completed; **24-16** (Docs-Truth Close + API Docs Expansion, BLOCKING pre-merge FIRST) and **24-17** (Deployment Setup (PR Env Scope) + Env-Isolation Hardening + IaC, pre-merge SECOND) are authored + **in-progress**, both before the single merge.
