...
# project-level cloudtexttospeech.user / aiplatform.user so preview builds can
# exercise the same external services as prod. ONE key = the single
# `GCP_PREVIEW_SA_KEY` GitHub secret consumed by .github/workflows/preview.yml.

resource "google_service_account" "preview" {
  account_id   = "preview-service"
  display_name = "Preview Service Account"
  description  = "PR preview access: sandbox bucket objectAdmin + TTS/Gemini users (single GCP_PREVIEW_SA_KEY)"
}
