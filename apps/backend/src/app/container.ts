...
const rawTtsService = {
  async synthesizeSpeech(text: string, options: Record<string, any>) {
...
const cachedAIFeedbackFn: any = withCache(
  (params: any) => aiFeedbackService.generateFeedback(params),
  {
    ttl: 86400,
...
export const cachedAIFeedback = {
  ...
  getMetrics: (cachedAIFeedbackFn as any).getMetrics,
};

// TTS: wrap raw TTS service with Redis cache (24h TTL)
const cachedTtsFn: any = withCache(
  (text: string, options: any = {}) => rawTtsService.synthesizeSpeech(text, options),
  {
    ttl: 86400,
    keyFn: (text: string, options: any) => `tts:${text}${options.voice || ""}`,
    serviceName: "TTS",
  },
);
...
export const cachedTts = {
  ...
  getMetrics: (cachedTtsFn as any).getMetrics,
};
