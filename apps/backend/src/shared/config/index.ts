 ...
 */
function parseJsonEnv(envVar: string, required: boolean = false): any | null {
  ...
  try {
    ...
    throw new Error(`[Config] ${envVar} contains invalid JSON: ${(error as any).message}`);
  }
}
 ...
 */
function validatedEnv(name: string): string {
...
function parseRedisUrl() {
  ...
  try {
    ...
    configLogger.error("Invalid REDIS_URL format", (error as any).message);
