/**
 * @file apps/backend/src/shared/infrastructure/external/GeminiService.ts
 * @description Application-level AI service wrapping GeminiClient.
 *
 * External Service layer (Tier 1) — adds timeout, retry, fallback, and
 * sanitization on top of the raw GeminiClient (Tier 0 external client layer).
 * Business-free and parametric: this is a shared resilient wrapper, not a
 * capability module (the 5-criteria test does not trip — see
 * docs/knowledge-base/backend/backend-shared-kernel-layer.md).
 *
 * Relocated from the retired shared services directory (shared now holds zero
 * capability logic).
 *
