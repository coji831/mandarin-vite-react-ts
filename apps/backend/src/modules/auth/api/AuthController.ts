/**
 * @file apps/backend/src/modules/auth/api/AuthController.js
...
export class AuthController {
  ...
  async register(req: Request, res: Response) {
    ...
    try {
      ...
      logger.error("Failed to register user", { error: error.message, email, ip: req.ip });
      res.status(500).json({
        error: "Failed to register user",
  ...
  async login(req: Request, res: Response) {
    ...
    try {
      ...
      logger.error("Failed to authenticate", { error: error.message, email, ip: req.ip });
      res.status(500).json({
        error: "Failed to authenticate",
        code: "LOGIN_FAILED",
        message: "An unexpected error occurred",
