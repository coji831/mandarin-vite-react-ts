...
export class AIFeedbackController {
  ...
  /**
   * @param feedbackService - AI Feedback service instance
   */
  ...
  async generateAIFeedback(req: Request, res: Response) {
    try {
      ...
      logger.error(`Error generating feedback: ${errMsg}`, { error });

      // Map errors to appropriate status codes
      if (errMsg.includes("Word not found")) {
      ...
      if (errMsg.includes("Missing or invalid GEMINI_API_CREDENTIALS_RAW")) {
      ...
      if (errMsg.includes("Invalid input")) {
        return res.status(400).json({
          error: "Failed to generate feedback",
          code: "VALIDATION_ERROR",
          message: errMsg,
