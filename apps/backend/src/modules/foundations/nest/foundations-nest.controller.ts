 ...
 *
 * Same service delegation, same 2xx JSON, same 4xx `message` (the global 24-3
 * `AppExceptionFilter` serializes thrown `HttpException`s into the
 * `{ code, message, requestId }` envelope). The previous surface emitted PLAIN
 * `{ error }` bodies (no `code`), so the Nest envelope supplies a `code` per
 * the backend error-message convention (`VALIDATION_ERROR` / 400, `NOT_FOUND`
 * / 404, `LOAD_ERROR` / 500) while `message` is byte-for-byte equal to the
 * previous `error` text — the established envelope-vs-legacy contract.
 *
 * FOUR routes — one controller with `@Controller("v1")` covering both route
 * prefixes the route file spans:
 ...
 *
 * ROUTE-SHADOWING PARITY (`GET /v1/characters/:glyph`): this route lives on the
 * FOUNDATIONS controller, which is mounted BEFORE the characters controller
 * (Nest registers routes in module import order onto the same Express router;
 * first-match-wins, no conflict detector fires) — so foundations'
 * `characters/:glyph` captures every single-segment `GET /v1/characters/<x>`
 * on the served route tree, shadowing the characters module's own `:glyph`
 * (+ its `/search` / `/frequency`) handlers. Result: `GET /v1/characters/search`
 * / `/frequency` / `<non-CJK>` return the foundations 404 `Character "<x>"
 * not found`, byte-for-byte. The characters `:glyph` handler remains
 * registered (later) and is reactivated only if foundations is ever removed
 * or reordered.
 *
 * The `getCharacterByGlyph` 400 branch (`!glyphParam || Array.isArray(...)`)
 * is ported for controller fidelity but is UNREACHABLE over HTTP — the
 * `:glyph` param is a required single-segment param, so it is always a string.
 */
