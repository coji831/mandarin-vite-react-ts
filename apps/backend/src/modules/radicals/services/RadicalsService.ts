...
export class RadicalsService {
   ...
   */
  async getAllRadicals(): Promise<ContentFile[]> {
    try {
   ...
   */
  async getRadicalById(radicalId: string): Promise<ContentFile> {
    try {
   ...
   * @returns Array of radical data objects
   */
  async getRadicalsByCharacter(glyph: string): Promise<ContentFile[]> {
    try {
