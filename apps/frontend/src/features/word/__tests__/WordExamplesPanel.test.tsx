...
describe("WordExamplesPanel", () => {
  ...
  it("renders skeleton while loading", () => {
    ...
    render(<WordExamplesPanel word="饭" hskLevel={1} language="en" />);
  ...
  it("plays audio when Play button clicked", async () => {
    vi.spyOn(examplesApi, "fetchExamples").mockResolvedValue(mockExamples as any);
    vi.spyOn(audioService, "getAudioUrl").mockResolvedValue("data:audio/mp3;...");
    vi.spyOn(audioService, "playAudio").mockResolvedValue();
    ...
    const playButtons = screen.getAllByRole("button", { name: /Play example/ });
  ...
  it("tracks analytics events", async () => {
    ...
    const playButton = screen.getByRole("button", { name: /Play example 1/ });
  ...
  it('renders max 5 examples with "Show more" button', async () => {
    ...
    render(<WordExamplesPanel word="多" hskLevel={1} language="en" />);
