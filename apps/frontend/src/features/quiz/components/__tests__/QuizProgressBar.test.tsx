/**
 * Tests for QuizProgressBar component
 * Story 15.6: Quiz Container & State Management
 */

import { describe, it, expect } from "vitest";
import { render, screen } from "@testing-library/react";
import { QuizProgressBar } from "../../components/QuizProgressBar";

describe("QuizProgressBar", () => {
  it("renders current and total count", () => {
    render(<QuizProgressBar current={2} total={5} />);
    expect(screen.getByText("2 / 5")).toBeInTheDocument();
  });

  it("calculates percentage correctly", () => {
    const { container } = render(<QuizProgressBar current={1} total={4} />);
    const progressFill = container.querySelector(".progressBarFill");
    expect(progressFill).toHaveStyle({ width: "25%" });
  });

  it("shows 0% for first question not yet answered", () => {
    const { container } = render(<QuizProgressBar current={0} total={3} />);
    const progressFill = container.querySelector(".progressBarFill");
    expect(progressFill).toHaveStyle({ width: "0%" });
  });

  it("shows 100% when all questions completed", () => {
    const { container } = render(<QuizProgressBar current={3} total={3} />);
    const progressFill = container.querySelector(".progressBarFill");
    expect(progressFill).toHaveStyle({ width: "100%" });
  });

  it("renders progress bar structure", () => {
    const { container } = render(<QuizProgressBar current={1} total={3} />);

    const progressContainer = container.querySelector(".progressContainer");
    const progressBarBackground = container.querySelector(".progressBarBackground");
    const progressBarFill = container.querySelector(".progressBarFill");

    expect(progressContainer).toBeInTheDocument();
    expect(progressBarBackground).toBeInTheDocument();
    expect(progressBarFill).toBeInTheDocument();
  });
});
